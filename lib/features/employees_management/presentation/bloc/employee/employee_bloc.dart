...
class EmployeeBloc extends Bloc<EmployeeEvent, EmployeeState> {
  ...
  EmployeeBloc(this._getEmployeesUseCase, this._insertEmployeeUseCase, this._deleteEmployeeUseCase,
      this._deleteEmployeesUseCase, this._exportEmployeesUseCase)
  ...
  void onGetEmployees(SearchEmployeeEvent event, Emitter<EmployeeState> emit) async {
  ...
  void onInsertEmployee(InsertEmployeeEvent event, Emitter<EmployeeState> emit) async {
    await _insertEmployeeUseCase(params: event.employee);
    emit(const InsertEmployeeSuccessState());
  }

  void onDeleteEmployee(DeleteEmployeeEvent event, Emitter<EmployeeState> emit) async {
    await _deleteEmployeeUseCase(params: event.employee);
    emit(const DeleteEmployeeSuccessState());
  }

  void onDeleteEmployees(DeleteEmployeesEvent event, Emitter<EmployeeState> emit) async {
    await _deleteEmployeesUseCase();
    emit(GetEmployeesSuccessState(List.empty()));
  }

  void onExportEmployees(ExportEmployeesEvent event, Emitter<EmployeeState> emit) async {
